...
@ManagedBean(name="lancamentoCaixaCriancasBean")
@ViewScoped
public class LancamentoCaixaCriancasBean {
...
//	private List<Compra>listaComprasLancamento;
	...
	/*public void pegarGrupoSelecionado(){
		nomeGrupo=lancamento.getGrupo().getNome();
		codigoGrupo=lancamento.getGrupo().getCodigo();
	...
	*/
	...
	public void listar() {
		try {
			...
			//lancamentos=ldao.buscarLancamentoPorGrupo("varões");
	...
	public void salvar() {
		...
		try {
			LancamentoDao ldao=new LancamentoDao();
			Grupo grupopesquisado=gdao.buscar(3L);
